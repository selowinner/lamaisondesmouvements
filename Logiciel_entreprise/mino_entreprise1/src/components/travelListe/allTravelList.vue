    ...
    <div class="tableWrapperDiv">
        ...
        <v-card>
            ...
            <v-container>
                <div class="CancelVerification">
                  ...
                  d'annulation vous devriez rembourser les tickets déjà achetés <br> <br>
                    <span style="font-weight:bold;">voulez-vous vraiment annuler <br> ce voyage ?</span> 
                </div>
        ...
        <v-card>
            ...
            <v-container>
                ...
                <div class="CancelVerification">
                    voulez-vous vraiment annuler <br> ce voyage ?
                </div>
                <div class="verificationAction">
                <v-btn color="Titlecolor" rounded depressed @click="closeDelete" style="color:white">Non</v-btn>
                <v-btn color="mainGreenColor" rounded depressed @click="deleteItemConfirm" style="color:white">Oui</v-btn>
        ...
        <v-card>
                ...
                <form class="updateForm">
                    <v-container fluid>
                        <v-row>
                          ...
                          <v-text-field  solo label="Append" append-icon="mdi-map-marker" v-model="travelToEdit.departure_place" ref="dep_place"   type="text"   value="Lieu de départ"  persistent-hint required disabled></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field background-color="#3e886d4a" solo  v-model="travelToEdit.departure_date" ref="dep_date"   type="date"   value="" label="Date de départ" persistent-hint required></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field   solo  append-icon="mdi-map-marker"  v-model="travelToEdit.destination" ref="dest_place" type="text"   label="Destination" persistent-hint required></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field  background-color="#3e886d4a" solo   v-model="travelToEdit.departure_time" ref="dep_time" type="time"   label="Heure départ" persistent-hint required></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field   solo  append-icon="mdi-car-seat"  v-model="travelToEdit.total_car_place_number" ref="pla_number" type="number"   label="Nombre de place à disponible"  persistent-hint required></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field  background-color="#3e886d4a" solo  append-icon="mdi-clipboard-text"  v-model="travelToEdit.car_informations" ref="car_infos" type="text"   label="type de car. ex: climatisé, dernière generation ..." persistent-hint required></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field   solo  append-icon="mdi-numeric"  v-model="travelToEdit.car_matriculation" ref="car_matri" type="text"   label="Matricule du car" persistent-hint required></v-text-field>
                        </v-col>
                        <v-col cols="12" md="12" lg="12">
                          <v-text-field  background-color="#3e886d4a" solo  append-icon="mdi-cash-multiple"  v-model="travelToEdit.place_price" ref="pl_price" type="number"   label="Prix du ticket"  persistent-hint required></v-text-field>
                        </v-col>

                        <v-col cols="12" md="12" lg="12">
                          <v-text-field   solo  append-icon="mdi-bus-stop"  v-model="travelToEdit.Intemediatestation" ref="st_inter" type="text"   label="atation intermediaires"  persistent-hint required></v-text-field>
                        </v-col>
                        </v-row>
                    </v-container>
                </form>
            ...
            <v-btn color="mainGreenColor" rounded depressed @click="editTravelConfirm" style="color:white">Enregistrer</v-btn>
        ...
        <v-alert v-if="addingSuccess" elevation="13" type="success" max-width="300" class="alert" color="mainGreenColor">{{this.travelDeleteResponse.message}}</v-alert>
        </transition>
        <transition name="slide"> 
        <v-alert v-if="addingfalse" elevation="13" type="error" max-width="300" class="alert" color="error">{{this.travelDeleteResponse.message}}</v-alert>
        </transition>
    </div>
...
export default {
  ...
  // --------------------
    ...
    deleteItemConfirm () {
      Vue.prototype.$http.put('http://127.0.0.1:3333/travel/cancellation', this.travelToDelete)
                .then(response =>{
                  ...
                  if (this.travelDeleteResponse.message == "success" ) {
                    ...
                    this.travelDeleteResponse.message = "Impossible d'effectuer l'annulation"
    ...
    editTravelConfirm () {
      ...
      Vue.prototype.$http.put('http://127.0.0.1:3333/travel/update', this.fielsdChanged)
                .then(response =>{
                  this.travelDeleteResponse = response.data
                  if (this.travelDeleteResponse.message == "success" ) {
