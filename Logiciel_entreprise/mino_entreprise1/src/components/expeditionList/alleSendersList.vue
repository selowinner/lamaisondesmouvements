<template>
    ...
    <div class="tableWrapperDiv">
        ...
        <v-data-table dense :headers="headers" :items="Senders" :search="search"  :items-per-page="-1" hide-default-footer>
        ...
        <v-btn  icon color="mainGreenColor"  @click="showItem(item)"><v-icon small v-if="item.activation_state == 1"> mdi-eye </v-icon> <v-icon small v-if="item.activation_state == 0" color="error"> mdi-account-alert </v-icon> </v-btn>
        <v-btn  icon color="mainGreenColor"  @click="editItem(item)" v-if="item.activation_state == 1"><v-icon small> mdi-lead-pencil </v-icon></v-btn>
        <v-btn  icon color="mainGreenColor"  @click="deleteItem(item)" v-if="item.activation_state == 1"><v-icon small> mdi-trash-can </v-icon></v-btn>
        </template>
        <!-- <template v-slot:[`item.contact`]="{ item }"> 
        <v-icon dense color="mainGreenColor"> mdi-phone </v-icon> <span style="color: mainGreenColor;">{{item.contact}}</span>
        </template> -->
        <template v-slot:[`item.complet_name`]="{ item }"> <!-- modification avec CESINHIO  a la base on avait v-slot:[item.actions="{ item }"-->
