<template>
  <div class="bodyBox">
    <div class="TheBoxBody ForcompanyDeclaration">
      ...
      <div>
        <div class="formRadio">
          <v-radio-group v-model="row" row dense mandatory>
            <v-radio
              label="ENREGISTRER UNE COMPAGNIE"
              value="radio-1"
              v-on:click="DayType = true"
            ></v-radio>
            <v-radio
              label="LISTE DES COMPAGNIES"
              value="radio-2"
              v-on:click="DayType = false"
            ></v-radio>
          </v-radio-group>
        </div>
      </div>
      ...
      <v-form
        ref="companiesaddingform"
        v-if="DayType"
        class="mainContentWraper"
      >
        <p>
          Lorem, ipsum dolor sit amet consectetur adipisicing elit. Sit
          reprehenderit atque voluptatem animi architecto nihil, iste, rerum
          maiores.
        </p>
        <v-container fluid>
          <v-row>
            <v-col cols="12" md="4" lg="4">
              <v-text-field
                outlined
                v-model="newCompany.denomination"
                ref="deno"
                height="50"
                type="text"
                :rules="[() => !!newCompany.denomination]"
            ...
            <v-col cols="12" md="4" lg="4">
              <v-text-field
                outlined
                v-model="newCompany.anagramme"
                ref="anag"
                height="50"
                type="text"
                :rules="[() => !!newCompany.anagramme]"
                label="anagramme"
                persistent-hint
                required
              ></v-text-field>
            ...
            <v-col cols="12" md="4" lg="4">
              <v-text-field
                outlined
                v-model="newCompany.matriculation"
                ref="rccm"
                height="50"
                type="text"
                :rules="[() => !!newCompany.matriculation]"
                label="Numéro RCCM"
                persistent-hint
                required
              ></v-text-field>
            ...
            <v-col cols="12" md="4" lg="4">
              <v-text-field
                outlined
                v-model="newCompany.siege"
                ref="siege"
                height="50"
                type="text"
                :rules="[() => !!newCompany.siege]"
                label="Ville de siège"
                persistent-hint
                required
              ></v-text-field>
            </v-col>
    ...
    <transition name="slide">
      <v-alert
        ...
        class="alert"
        color="mainGreenColor"
        >{{ this.companyaddingResponse.message }}</v-alert
      >
    </transition>
    <transition name="slide">
      <v-alert
...
export default {
  ...
  methods: {
    submit() {
      axios
        .post("http://127.0.0.1:3333/compangny/add", this.newCompany)
        .then((response) => {
          this.companyaddingResponse = response.data;
          console.log(response.data);
          if (this.companyaddingResponse.message == "success") {
            this.companyaddingResponse.message =
              "Compagny enregistrée avec succès";
